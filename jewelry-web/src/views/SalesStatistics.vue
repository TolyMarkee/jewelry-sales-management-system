<template>
  <div class="space-y-6">
    <h2 class="text-lg font-bold" style="color:var(--text-primary)">销售统计</h2>

    <!-- Stat cards -->
    <div class="grid grid-cols-4 gap-4">
      <div v-for="(s,i) in statCards" :key="i" class="rounded-2xl p-5 flex items-center gap-4" style="background:var(--surface); border:1px solid var(--border)">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center text-lg" :style="{background:s.bg, color:s.color}">{{ s.icon }}</div>
        <div><div class="text-2xl font-bold" :style="{color:s.color}">{{ s.value }}</div><div class="text-xs" style="color:var(--text-muted)">{{ s.label }}</div></div>
      </div>
    </div>

    <!-- Charts -->
    <div class="flex gap-4">
      <div class="flex-1 rounded-2xl p-5" style="background:var(--surface); border:1px solid var(--border)">
        <div class="text-sm font-semibold mb-4" style="color:var(--text-primary)">订单状态分布</div>
